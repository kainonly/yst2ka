...
- 生成方案以网站文档为主、图片识别为辅。优先读取官网具体接口页中的请求 path、`transCode`、业务请求参数、业务响应参数；截图、表格图片、聊天中转述内容仅在官网页面缺失、排版损坏或字段备注无法完整提取时作为补充兜底。
- 如果网站文档与图片识别结果不一致，以官网具体接口页为准；图片只用于补全官网页未能稳定提取的字段名称、说明或示例，不用于推翻官网页里的交易号、请求路径、字段名和字段类型。
...
- 如果不同文档页复用同一个 `transCode`，应合并到同一个交易文件中，再用场景化构造函数、setter 或辅助方法表达差异，不要因为文档页不同重复生成同交易号文件。例如 1881【自带参数的当面付订单支付H5】与 2166【自带参数的当面付快捷H5】同为 `transCode=4013`，应统一落在 `Tx4013`。
- 纯通知页如果页面有显式 `transCode`，可以生成对应交易号的 `XXXNotifyResult` 模型；如果页面没有显式 `transCode`，默认按“通知订阅页”处理，直接忽略，不要臆造交易号、文件名或待实现任务，例如 686、2603、2681。
- 如果页面只是接入说明、方案说明或目录分流页，且没有明确请求 path / `transCode`，不要直接生成交易文件；应继续跟随页面内链接定位真正的接口页。例如 1115【微信支付分停车服务（直联模式）】是接入说明，不是独立交易接口。
- 如果具体接口页当前无法稳定获取正文参数表，不要猜测字段或交易结构；先直接忽略，不纳入实现范围，例如 2137。
- 当前仓库已实现页面与文档地址可按目录记忆：659 目录相关为 Tm1010->663、Tm1011->664、Tm1012->2070、Tm1013->2072、Tm1014->2429、Tm1015->2605、Tm1020->661、Tm1022->662、Tm1023->673、Tm1024->676、Tm1025->671、Tm1026->675、Tm1027->672、Tm1029->1606、Tm1030->665、Tm1031->667、Tm1032->666、Tm1033->852、Tm1035->1884、Tm1043->2263、Tm1050->668、Tm1051->669、Tm1053->1075、Tq1062->1003、Tm4001->677、Tm4043->2197、Tq3004->674；678 目录相关为 Tx2085->679、Tx2086->2344、Tx2089/Tx2090/Tx2091->680、Tx2084->681、Tx2099->2601、Tx2290->682、Tx2096->1516、Tx2294->683、Tx2295->684、Tx2094->859、Tx2293->858、Tx2286->2346、Tx3010->685、Tx4006->874、Tx4013->1881/2166、Tx4016->2547、Tx4023->2553、Tx4037->2341、Tm2299->2723；688 目录相关为 Tq3001->689、Tq3002->690、Tq3007->2602、Tq4003->691。
